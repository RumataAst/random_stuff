...
void                    printStats(const std::vector<double> &distances, struct distanceStatistic stat_dist);
...
bool    parseDouble(const std::string &user_input, std::vector<double> &distances) {
    ...
    while (input_stream >> num_str) {
        if (std::ranges::count(num_str, '.') != 1 ||std::ranges::count(num_str, '.') != 0) return false;
        if (!safe_stod(num_str)) return false;
        num = *safe_stod(num_str);
...
std::optional<double> safe_stod(const std::string& input) {
    try {
        ...
        std::cout << "Error: not a number" << e.what() << std::endl;
